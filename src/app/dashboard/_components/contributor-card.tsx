...
const ContributorCard = ({ initContributorList, dao, githubToken }: ContributorCardProps) => {
  ...
  const { data: ownerData, isPending: isOwnerPending } = api.contributor.owner.useQuery({ daoId: dao.id })
              ...
              <div key={`Contributor-${item.id}`} className={"flex flex-row items-center justify-between gap-2 font-thin"}>
                <div className={"flex flex-row items-center gap-2"}>
                  ...
                  {item.userAddress !== null ? (
                    <a href={`${defaultChain.blockExplorers.default.url}/address/${item.userAddress}`} className={"text-muted-foreground text-xs hover:text-white transition-colors cursor-pointer"} target="_blank" rel="noopener noreferrer">{shortenAddress(item.userAddress)}</a>
                  ) : (
