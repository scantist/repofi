        ...
        <div className={"mt-3 flex flex-col gap-2"}>
          {isPending ? (
            ...
            top10Contributors?.map((item, index) => (
              <div key={`Contributor-${item.id}`}
                   className={"flex flex-row items-center justify-between gap-2 font-thin"}>
                <div className={"flex flex-row items-center gap-2"}>
                  ...
                  {item.userAddress !== null ? (
                    <a href={`${defaultChain.blockExplorers.default.url}/address/${item.userAddress}`}
                       className={"text-muted-foreground text-xs hover:text-white transition-colors cursor-pointer"}
                       target="_blank" rel="noopener noreferrer">{shortenAddress(item.userAddress)}</a>
                  ) : (
                    <div className={"text-muted-foreground text-xs cursor-pointer"}/>
                  )}
                  {ownerData && item.userAddress === ownerData.userAddress && (
                    <div
                      className={"border border-primary rounded-lg text-xs px-2 py-1 text-secondary font-bold"}>You</div>
                  )}
                </div>
                <div
                  className={"bg-primary ml-4 rounded-lg px-2 py-1 text-right text-xs opacity-80"}>{Number(item.snapshotValue).toFixed(2)}%
                </div>
              </div>
            ))
          )}
