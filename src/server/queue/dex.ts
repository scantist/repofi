...
const syncAssetPrice = async (logger: typeof console) => {
  try {
    ...
    throw new CommonError(ErrorCode.INTERNAL_ERROR, "Failed to sync asset prices", error)
  }
}
...
const syncLaunchingDaoMetrics = async (logger: typeof console) => {
  try {
    ...
    const updates = launchingDaoTokens
      .map(async (tokenInfo) => {
    ...
    await Promise.all(updates)
    logger.info(`Successfully synced prices and market caps for ${updates.length} launching DAO`)
  } catch (error) {
    logger.error("Failed to sync launching DAO prices and market caps:", error)
    throw new CommonError(ErrorCode.INTERNAL_ERROR, "Failed to sync launching DAO prices and market caps", error)
  }
}
...
const syncGraduatedDaoMetrics = async (logger: typeof console) => {
  try {
    ...
    const updates = graduatedDaoTokens
      .map(async (tokenInfo) => {
      ...
      .filter((update) => update !== undefined)
    await Promise.all(updates)
    logger.info(`Successfully synced prices and market caps for ${updates.length} graduated DAO`)
  } catch (error) {
    logger.error("Failed to sync graduated DAO prices and market caps:", error)
    throw new CommonError(ErrorCode.INTERNAL_ERROR, "Failed to sync graduated DAO prices and market caps", error)
  }
}
