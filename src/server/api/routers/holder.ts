...
export const holderRouter = createTRPCRouter({
  getTop10Holders: publicProcedure.input(z.object({ tokenId: z.bigint() })).query(({ input }): Promise<Top10Holders> => {
    return holderService.getTop10Holders(input.tokenId)
  }),
  getHolders: publicProcedure.input(z.object({ tokenId: z.bigint() }).merge(pageableSchema)).query(async ({ input }) => {
